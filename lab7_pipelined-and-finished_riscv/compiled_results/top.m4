...
\TLV
   ...
   |cpu
      @0
         $reset = *reset;
         // Next PC
         $pc[31:0] = >>1$reset ? '0 : >>3$valid_taken_br ? >>3$br_tgt_pc : >>3$valid_load ? >>3$inc_pc : >>3$valid_jump && >>3$is_jal ? >>3$br_tgt_pc :>>3$valid_jump && >>3$is_jalr ? >>3$jalr_tgt_pc : >>1$inc_pc;
      ...
      @2
         // Register File Read
         $rf_rd_en1 = $rs1_valid; // oops corrected: was enl should be en1
         $rf_rd_en2 = $rs2_valid;
   ...
   *failed = 1'b0;
   ...
   //  o CPU visualization
   |cpu
      \source /raw.githubusercontent.com/BalaDhinesh/RISCVMYTHWorkshop/master/tlvlib/riscvshelllib.tlv 19   // Instantiated from top.tlv, 252 as: m4+imem(@1)
      ...
      \source /raw.githubusercontent.com/BalaDhinesh/RISCVMYTHWorkshop/master/tlvlib/riscvshelllib.tlv 74   // Instantiated from top.tlv, 253 as: m4+rf(@2, @3)
         // Reg File
         @3
            /xreg[31:0]
      ...
      \source /raw.githubusercontent.com/BalaDhinesh/RISCVMYTHWorkshop/master/tlvlib/riscvshelllib.tlv 91   // Instantiated from top.tlv, 254 as: m4+dmem(@4)
         // Data Memory
         @4
            /dmem[15:0]
